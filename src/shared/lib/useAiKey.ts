...
/**
 * 저장된 키를 제공자 구분 없이 모두 지운다.
 *
 * 로그아웃 시 자동 호출했었지만 지금은 부르지 않는다 — 같은 브라우저에서도 로그아웃할
 * 때마다 다시 등록해야 했기 때문이다. 해제는 AI 조사 설정 화면에서 사용자가 직접 한다.
 *
 * 키는 계정이 아니라 브라우저에 붙어 있으므로, 공용 PC 에서는 로그아웃 후에도 키가
 * 남는다는 점을 감수한 결정이다. 공용 기기 대응이 다시 필요해지면 이 함수를 그 경로에
 * 붙이면 된다.
 *
 * 훅 밖에서도 부를 수 있게 컴포넌트가 아닌 함수로 둔다.
 */
