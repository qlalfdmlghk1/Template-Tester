          ...
          <div className="flex w-full flex-row gap-2">
            <div className="w-full space-y-3">
              ...
              <div className="border border-border rounded-lg p-3 bg-surface">
                {note.myCodeLabel && (
                  <div className="mb-2">
                    <span className="inline-block px-2 py-0.5 text-xs font-medium bg-blue-50 text-primary rounded">
                      {note.myCodeLabel}
                    </span>
                  </div>
                )}
            ...
            {note.solutions.length > 0 && (
              <div className="w-full space-y-3">
                <label className="block text-sm font-medium text-text leading-[30px]">참조한 풀이 ({note.solutions.length}개)</label>
                {note.solutions.map((solution, index) => (
                  <div key={index} className="border border-border rounded-lg p-3 bg-surface">
                    ...
                    <CodeEditor value={solution.code} language={note.language} onChange={() => {}} readOnly />
                  </div>
                ))}
              </div>
            )}
