...
- 이슈: #72 (Fix)
- 브랜치: fix/72-coding-calendar
- 단계: 문구 수정 완료, 커밋·PR 대기
- 마지막 업데이트: 2026-07-29

---

## [Issue #72] 연동 도움말에 프로그래머스 안내 보강

**Type**: Fix | **Jira**: (미사용) | **시작**: 2026-07-29

### ✅ 완료

- [x] 작업 환경 셋업 (/start 실행)
- [x] 패널 상단에 대상 플랫폼(프로그래머스·백준) 요약 문장 추가
- [x] 준비물 문구를 플랫폼 주어로 재작성
- [x] `읽는 것` 섹션에 프로그래머스·백준 커밋 예시 분리 표기
- [x] 플랫폼 판별 규칙(`level 2` vs `Silver III`) 안내 추가
- [x] `문제 제목에 링크가 없다면` 섹션에 플랫폼별 링크 생성 방식·번호 색인 조건 명시

**검증 상태**: `npx eslint` 0건 / `npx tsc --noEmit --project tsconfig.app.json` 0건
...
- [2026-07-29] 문구에서 해명형 표현을 **배제** — "확장 이름 때문에 백준 전용으로 보이지만", "한쪽만 풀어도 상관없습니다" 같은 표현이 오히려 프로그래머스를 나중에 덧붙인 곁다리로 읽히게 만든다는 사용자 지적 반영
- [2026-07-29] 준비물 문장에서 **확장 이름을 주어 자리에서 내림** — "BaekjoonHub 확장이 설치돼 있고..." → "프로그래머스·백준 풀이가 BaekjoonHub 확장으로 자동 커밋되고 있어야 합니다". 이름에 백준이 들어가도 플랫폼이 주체로 읽히도록 함
- [2026-07-29] 확장 제품명 `BaekjoonHub`와 저장소 링크는 **유지** — 사용자가 실제로 설치할 때 찾아야 하는 이름
- [2026-07-29] "백준이 현재 닫혀 있다"는 상황은 **UI에 넣지 않음** — 일시적 상황이 문구로 굳으면 이후 더 혼란스러움
...
- `SyncRepoSetting` 미연동 안내 문구도 같은 문제("BaekjoonHub 확장이 풀이를 자동 커밋하는 공개 저장소...")가 있어 별도 처리 필요
- 헤더 모바일 레이아웃 이상 (`widgets/Navbar`) — 별도 이슈
...
### Commit — 2026-07-29 01:40

- Message: `Fix:#72 연동 도움말에 프로그래머스 안내 보강`
- Issue: `#72`
- Jira: (미사용)

**변경 요약**

연동 도움말(`SyncHelp`)이 BaekjoonHub·백준 위주로 쓰여 있어, 프로그래머스 풀이도 달력에 잡힌다는 사실이 사용자에게 드러나지 않던 문제를 고쳤다. 구현(`baekjoonHub.ts`)은 처음부터 두 플랫폼을 대등하게 파싱하고 있었으므로 문구만 조정했고, 파싱·저장 로직은 건드리지 않았다.

- 패널 상단에 대상 플랫폼(프로그래머스·백준)과 동작(채점 통과 → 자동 커밋 → 달력) 요약 문장 추가
- 준비물 문구에서 확장 이름을 주어 자리에서 내림 — "프로그래머스·백준 풀이가 BaekjoonHub 확장으로 저장소에 자동 커밋되고 있어야 합니다"
- `읽는 것` 섹션의 커밋 예시를 플랫폼별로 라벨과 함께 분리 (기존에는 프로그래머스 예시 하나가 라벨 없이 있어 단순 샘플로 읽혔다)
- 플랫폼 판별 규칙 안내 추가 — `level 2`면 프로그래머스, `Silver III` 같은 티어면 백준 (`parseBaekjoonHubMessage`의 분기와 일치)
- `문제 제목에 링크가 없다면` 섹션에 링크가 비는 조건 명시 — 최상단 폴더가 `프로그래머스`/`백준`이어야 하고, 문제 폴더가 `번호. 제목` 형태여야 함 (`buildProblemIndex` 기준)

**결정 로그**

- 해명형 표현을 **배제** — "확장 이름 때문에 백준 전용으로 보이지만", "한쪽만 풀어도 상관없습니다"는 오히려 프로그래머스를 나중에 덧붙인 곁다리로 읽히게 만든다는 사용자 지적을 반영. 두 플랫폼을 처음부터 하나의 대상으로 서술하는 쪽으로 바꿈
- "모두 읽습니다"의 **"모두"도 제거** — 열거로 읽히면 두 플랫폼이 별개 대상이 된다. 대신 기능의 실제 기준인 "채점을 통과한 풀이"를 문장의 주어로 올림
- 확장 제품명 `BaekjoonHub`와 저장소 링크는 **유지** — 사용자가 실제로 설치할 때 찾아야 하는 이름이라 바꿀 수 없음. 대신 문장 구조에서 수단 자리로 내려 백준 전용으로 읽히지 않게 함
- "백준이 현재 닫혀 있다"는 상황은 **UI에 넣지 않음** — 일시적 상황이 문구로 굳으면 이후 더 혼란스러움
- 백준 예시는 실재가 확실한 `1000. A+B`(Bronze V)를 사용 — 문서에 존재하지 않는 문제번호·티어 조합이 남지 않도록

**다음 작업**

- `SyncRepoSetting`의 미연동 안내 문구도 같은 문제("BaekjoonHub 확장이 풀이를 자동 커밋하는 공개 저장소...")가 있어 별도 처리 필요 — 첫 연동 사용자가 가장 먼저 보는 문구
- (#67에서 이월) firestore.rules 재게시 필요 — 현재 운영 규칙은 삭제가 거부되는 상태
