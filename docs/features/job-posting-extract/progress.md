...
### Commit — 2026-08-15 03:08

- Message: `Fix:#92 캡처 상한 안내를 탭 밖으로 — 본문 탭에서 안 보이던 문제`
- Issue: `#92`

**변경 요약**

- `imageError` 렌더를 탭 패널 밖 공통 영역으로 이동
- 자리가 없으면 파일을 읽기 전에 조기 반환

**결정 로그**

- 직전 커밋이 `setMode("image")` 게이트를 `accepted.length > 0` 으로 좁히면서, 목록이 꽉 찬 상태로 **본문 탭**에서 붙여넣으면 탭 전환이 없어 안내가 화면에 뜨지 않았다. 안내가 이미지 탭 패널 안에만 렌더됐기 때문이다. 패널 밖으로 올리면 읽기 실패(`failure`) 경로까지 함께 덮인다.
- **이 루프에서 수정이 회귀를 만든 것이 3라운드 연속이다** (Round 1 → 상한 안내 누락, Round 2 → 본문 탭 표시 누락). 전부 `PostingPasteDialog` 의 상태·표시 결합 지점에서 났다. 이 컴포넌트에 렌더 레벨 테스트가 없는 것이 근본 원인이며, 최우선 후속 과제다.

**다음 작업**

- `PostingPasteDialog.spec.tsx` — 탭 전환·상한·읽기 실패 표시
- 캡처 인식률 실측
