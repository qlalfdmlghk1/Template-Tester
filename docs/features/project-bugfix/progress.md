...
- Message: `Feat:#85 공고 링크를 지원 건 단위로 입력·저장`
- Issue: `#85`
- Jira: 미사용

**변경 요약**

- `JobApplication`에 `postingUrl` 추가. 등록 폼의 공고 링크 입력을 "새 기업 등록" 블록 밖으로 꺼내 공고명 아래에 상시 노출 — 기존 기업을 골라도, 수정할 때도 링크를 넣을 수 있다
- 상세 패널은 지원 건 링크를 우선 쓰고 없으면 기업의 대표 링크로 대체
- 새 기업을 만들 때는 입력한 링크를 기업 대표 링크로도 함께 저장

**결정 로그**

- 링크를 기업이 아니라 지원 건에 둔다 — 공고명이 이미 지원 건 단위인데 링크만 기업 단위여서, 같은 기업에 여러 번 지원하면 나중 건이 앞의 링크를 덮어썼다
- `Company.postingUrl`은 제거하지 않는다. 기업 조사 화면이 쓰고 있고, 지원 건 링크가 비었을 때의 폴백으로도 의미가 있다
...
- Message: `Fix:#85 반기 판정 앵커를 자소서에서 "가장 이른 전형 일정"으로 확장`
- Issue: `#85`
- Jira: 미사용

**변경 요약**

- `getApplicationHalfId` 판정 순서를 `자소서 → 등록 시점`에서 `자소서 → 다른 전형 단계 중 가장 이른 일정 → 등록 시점`으로 확장
- `half.spec.ts`에 케이스 3건 추가 (다른 단계 일정 사용 / 여럿 중 가장 이른 것 / 자소서 우선)

**결정 로그**

- 리뷰에서 나온 "XLSX 임포트 건이 임포트 시점 반기로 빨려 들어가 합격률을 오염시킨다"에 대한 1단계 대응. 시트 행은 자소서가 비어도 코딩테스트·면접 일정이 있는 경우가 많아 대부분 이 단계에서 올바른 반기로 잡힌다
- 임포트 출처 플래그를 만들지 않았다 — "그 건에 대해 아는 가장 이른 날짜"라는 규칙이 임포트·수기 등록 양쪽에 똑같이 옳다
- 날짜가 하나도 없는 행(시트명 기반 추정)은 이번 범위에서 제외하고 별도 이슈로 분리 — 추정이 들어가는 영역이라 실제 데이터를 보고 정한다
