...
- Message: `Fix:#85 목록 카드 요약에 강조 표기가 그대로 노출되던 문제 수정`
- Issue: `#85`
- Jira: 미사용

**변경 요약**

- `/review-converge` 1라운드 리뷰에서 나온 Blocker 반영: `toPlainText()`를 추가하고 `CompanyResearchCard.summarize()`가 통과시키도록 수정. 프롬프트에 `**` 표기를 넣으면서 상세·미리보기만 갱신하고 목록 카드를 놓쳐 별표가 글자로 노출되던 문제
- "전체" 보기에서 지원 건을 저장하면 현재 반기로 시야가 좁아지던 동작 수정 — `activeHalfId !== ALL_HALF_ID`일 때만 이동
- `ResearchText`에 `as` prop 추가. 기본은 문단(`p`), 미리보기(`label > span` 내부)만 `span` — 상세 화면의 문단 시맨틱 복원
- `ALL_HALF_ID` 주석이 옛 규칙("자소서 일정이 없어 미분류")을 설명하던 것 정정

**결정 로그**

- 강조 표기를 걷어내는 책임은 `shared/lib/emphasis.ts`의 `toPlainText`에 둔다 — 앞으로 평문 경로(요약·툴팁·내보내기)가 늘어도 같은 함수를 태우면 된다
- 리뷰에서 나온 XLSX 임포트 반기 귀속 문제는 자동 반영하지 않고 escalate — 어떤 날짜를 기준으로 삼을지가 정책 판단이다

**다음 작업**

- XLSX 임포트 건의 반기 귀속 규칙 결정 (사용자 확인 대기)
